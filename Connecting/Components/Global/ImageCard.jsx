...
const ImageCard = ({ index, item, setSingleID, activeUser }) => {
  console.log(item);
  return (
    <div key={index} onClick={() => setSingleID
      (item?._id)} className="new-Card-width"
      >
        <div className="block relative group
        select-none overflow-hidden m-0.5
        ...
        >
          <a className={`absolute inset-0`}>
            ...
            pointer-events-none line-clamp px-2 pb-2
            text-sm px-4"
            style={{
              background:"linear-gradient(to top, rgba(0, 0, 0.8) 0%, ragba(0, 0, 0, 0) 60% rgba(0, 0, 0, 0) 100%)",

            }}
            >
              <div className="flex-shrink h-full flex
              items-end">
                <div className="flex flex-col">
                  ...
                  <p className="opacity-70 leading-snug text-xs mb-1"
                  style={{
                    overflow: "hidden",
                    display: "-webkit-box",
                    WebkitLineClamp: 3,
          ...
          <img className="pointer-events-none" src={`${DOMAIN_URL}${item?.images[0]}`}
          alt="" />
        </div>
      </div>
  );

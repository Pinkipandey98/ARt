...
const Header = () => {
  ...
  return (
  <div className="fixed w-screen bottom-0 sm:top-0 z-50 flex flex-row
  items-center justify-between backdrop-blur bg-opacity-80 border-t sm:border-t-0 sm:border-b
  border-opacity-50 text-sm select-none bg-zinc-900 border-t-zinc-700 sm:border-b-zinc-700"
  style={{
    height: 56,
  }}
  >
    <a href="/"
    className="hidden sm:flex items-center cursor-pointer px-4 pl-6
    left-0 h-full w-32">
      <HeaderLogo />
    </a>
    ...
    <div className="flex relative items-center h-full -mt-1 w-full sm:w-auto">
      ...
      />
        {navList.map((menu, index) =>(
          <a key={index}
          className="flex flex-row items-center cursor-pointer h-full py-1.5 pb-0
          px-2 justify-center transition-all flex-1"
          href={menu.link}
          style={{
            width: 80, 
            zIndex:2,
        ...
        {auth ? (
            ...
            <div className="hidden w-32 h-full sm:flex items-center justify-end mr-6">
              ...
              hover:brightness-110 px-4 text-xs md:text-sm bg-gradient-to-t
              from-indigo-800 via-indigo-800 to-indigo-600 drop-shadow font-medium
              whitespace-nowrap"
              >
                Get Started
              </a>
